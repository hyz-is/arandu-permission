...
func TestTheApplicationIntegrationHasOneCompositionFile(t *testing.T) {
	...
	guides := []string{
		"README.md",
		".agents/skills/permission-package/SKILL.md",
	}
	...
	for _, guide := range guides {
		...
		text := string(body)
		for _, required := range []string{"bootstrap/app.go", "permission.New(", "k.Register"} {
...
func TestTheGuidesTeachTheModelFirstBoundary(t *testing.T) {
	...
	guides := map[string][]string{
		"AGENTS.md":                {"Groups(db)", "security.Authorize"},
		"README.md":                {"Groups(db)", "Model terminal"},
		"CONTRIBUTING.md":          {"Groups(db)", "authorizes before reaching the Model"},
		"SECURITY.md":              {"Model terminal", "data.Tenant(g)"},
		".agents/skills/README.md": {"CRUD Repository", "configured Model"},
		".agents/skills/permission-module/SKILL.md":  {"func Groups", "security.Authorize"},
		".agents/skills/permission-policy/SKILL.md":  {"security.Authorize", "Groups("},
		".agents/skills/permission-release/SKILL.md": {"Model-first", "configured copy"},
	}
	for path, required := range guides {
		...
		for _, stale := range []string{"PermissionRepository", "NewPermissionRepository", "repository.go  data access"} {
