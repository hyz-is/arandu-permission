...
// TestTheArchiveSurvivesBeingPublished reads the module the way somebody who
// imports it does, and refuses a file the packer would leave behind.
//
// Every other gate here compiles the repository, where the files are present.
// The go command drops any path with a segment named "vendor" when it packs a
// module, so a source tree kept under one is in the repository and absent from
// what anybody downloads: the embed then matches nothing, and the first person
// to import the package gets "pattern resources/...: no matching files found"
// on a build that is green here.
//
