...
func module(t *testing.T) *permission.Module {
	t.Helper()
...
// TestTheRenderedNamesAreTheNamesTheArchiveCarries keeps the constants a
// handler renders by from drifting off the files that are published.
//
// They are two spellings of one name -- a constant in the package and a path in
// the archive -- and the module refuses to boot on the second, so without this
// a renamed file is found by starting an application rather than by running the
// suite.
func TestTheRenderedNamesAreTheNamesTheArchiveCarries(t *testing.T) {
	t.Parallel()
	...
	rendered := []string{
		permission.ViewGroupsIndex,
		permission.ViewGroupsShow,
	...
	for _, name := range rendered {
		if !published[name] {
			t.Errorf("a handler renders %q and no published file carries that name", name)
		}
	}
	if len(rendered) != len(published) {
		t.Errorf("%d views are published and %d are rendered: a published view nothing renders is a file an application maintains for nothing",
			len(published), len(rendered))
	}
}
...
func TestThePublicationLandsWhereTheModuleSaysItDoes(t *testing.T) {
	...
	slices.Sort(targets)
	if want := permission.PublishedPaths(); !slices.Equal(targets, want) {
...
func TestEveryPublishedFileLandsUnderTheModuleNamespace(t *testing.T) {
	...
	paths := permission.PublishedPaths()
	if len(paths) == 0 {
...
func TestTheNameAViewIsRenderedByComesFromItsPath(t *testing.T) {
	...
	packages := permission.ViewPackages()
	if len(packages) == 0 {
...
func TestTheModuleRefusesToBootUntilItsViewsAreLinked(t *testing.T) {
	...
	if !strings.Contains(err.Error(), permission.PublishCommand) {
		t.Errorf("the refusal does not say how to publish the views: %v", err)
	}
	for _, pkg := range permission.ViewPackages() {
	...
	for _, source := range productionGoFiles(t, packageRoot(t)) {
		if source.file.Name.Name == "main" && buildable(source.file) {
			t.Errorf("%s is a command this module carries; what a package publishes is declared, and %s writes it",
				source.path, permission.PublishCommand)
		}
	}
}
