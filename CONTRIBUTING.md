...
2. **Every Service method authorizes before reaching the Model.** The Grant is
   then required by the Model terminal; constructing `Groups(db)` first is
...
CRUD stays on `Groups(db)` and its Builder. Add a Repository only for a
...
licence, the same as the rest of github.com/hyz-is/arandu-permission.
