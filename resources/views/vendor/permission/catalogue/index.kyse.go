	...
	@if(len(.Domains) > 0)
		<div class="mt-8 grid gap-10">
			@foreach(.Domains as domain)
				<section>
					<h2 class="text-sm font-semibold tracking-tight uppercase">{{ .Labels.Domain(domain.Name) }}</h2>
					<ul class="mt-3 grid gap-2">
						@foreach(domain.Entries as entry)
							<li class="flex flex-wrap items-center justify-between gap-3 border-b py-2">
								<span class="flex flex-col">
									<span class="text-sm">{{ .Labels.Action(entry.Action) }}</span>
									<span class="text-muted-foreground font-mono text-xs">{{ string(entry.Action) }}</span>
								</span>
								<span class="flex flex-wrap items-center gap-1">
									@if(len(entry.Groups) > 0)
										...
										@endforeach
									@else
										<span class="text-muted-foreground text-xs">{{ .Labels.T("catalogue.nobody") }}</span>
									@endif
								</span>
							</li>
						@endforeach
