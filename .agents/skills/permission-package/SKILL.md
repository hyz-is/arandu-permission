...
| `GET` | `/permission/{id}` | `permission.show` |
| `POST` | `/permission` | `permission.store` |
...
`GET /permission` answers `{"data": {"items": [...]}}`, and adds
...
`GroupPolicy` denies every action and has no branch that allows one:
...
There are five actions — `PermissionView`, `PermissionList`, `PermissionCreate`,
`PermissionUpdate`, `PermissionDelete` — and opening one opens exactly one.
...
`github.com/hyz-is/arandu-permission`, which belongs to `hyz-is`. A vulnerability goes to the
