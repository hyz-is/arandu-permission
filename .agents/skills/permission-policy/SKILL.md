...
4. Only then does a `security.Grant` exist, and `service.go` reaches
   `Groups(s.db)` and a Model terminal with it.
...
`Groups(s.db)` call is therefore mandatory even though the terminal itself
also takes `g`.
...
```go
	// arandu:begin custom
	if a == PermissionView && (s.ID == record.ID || s.HasRole("admin")) {
		return nil
	}
	// arandu:end custom
```
...
```go
	if record.ID != "" && record.TenantID != s.Tenant {
		return fmt.Errorf("permission belongs to another tenant")
	}
```
...
`PermissionService` owns `db *data.DB`. Validate first, build the value the Policy
must see, call `security.Authorize`, and only then call `Groups(s.db)`.
...
```go
	g, err := security.Authorize(ctx, s.policy, actor, PermissionView, Permission{})
	record, err := Groups(s.db).NewQuery().WhereKey(id).First(ctx, g)
	_, err = security.Authorize(ctx, s.policy, actor, PermissionView, *record)
```
