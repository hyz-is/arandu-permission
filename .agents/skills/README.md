...
| skill | when it fires |
| --- | --- |
| `permission-policy` | opening an action, adding an authorized Model-backed use case, or anything answering 403 |
| `permission-module` | adding a route, a handler, a config field, a response field or a migration |
| `permission-release` | the gates, the manifest, a dependency, a version, a tag |
| `permission-vault-notes` | writing the note, the gap or the journal entry, when this checkout sits inside the Arandu Obsidian vault |
| `permission-package` | installing and wiring this package **into an application** |
...
`permission-vault-notes` fires on a condition rather than on a task: it applies
